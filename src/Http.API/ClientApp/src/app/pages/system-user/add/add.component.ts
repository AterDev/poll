...
export class AddComponent implements OnInit {
    ...
    get passwordSalt() { return this.formGroup.get('passwordSalt'); }
  ...
  initForm(): void {
    this.formGroup = new FormGroup({
      ...
      passwordHash: new FormControl(null, [Validators.maxLength(100)]),
      passwordSalt: new FormControl(null, [Validators.maxLength(60)]),
  ...
  getValidatorMessage(type: string): string {
    switch (type) {
      ...
      case 'passwordHash':
        return this.passwordHash?.errors?.['required'] ? 'PasswordHash必填' :
          this.passwordHash?.errors?.['minlength'] ? 'PasswordHash长度最少位' :
            this.passwordHash?.errors?.['maxlength'] ? 'PasswordHash长度最多100位' : '';
      case 'passwordSalt':
        return this.passwordSalt?.errors?.['required'] ? 'PasswordSalt必填' :
          this.passwordSalt?.errors?.['minlength'] ? 'PasswordSalt长度最少位' :
            this.passwordSalt?.errors?.['maxlength'] ? 'PasswordSalt长度最多60位' : '';
      case 'phoneNumber':
